...
**NOTE**: About the status of this format, the specification is still at `Draft` level, but the core concepts are finalized and stabilized. There won't be any breaking change since the 1.0.0 release of the specifications, which will come very soon. By the way, **contributions are welcome**!
...
- a JSON file `bundle.json` defining the application
- multiple invocation images, either Docker images or Virtual Machine images

### Bundle.json

The first artifact is the bundle file, defining the following parts:
...
The second artifact is the invocation image, which is a simple Docker image, able to install the application. It's not the application itself, see it as the installer binary.
...
1. Create environment variable with the installation name (`CNAB_INSTALLATION_NAME`), bundle name (`CNAB_BUNDLE_NAME`), bundle version (`CNAB_BUNDLE_VERSION`), and finally the action to execute (`CNAB_ACTION`)
1. Either create environment variables or mount files for parameters
1. Mount with `tmpfs`the credentials to the specified paths
...
Let's put our hands on what we just saw an create our first CNAB!

1. Create a new directory called `simple`
1. In it, copy/paste the previous bundle example in a new file `bundle.json`
...
**EXERCISE**: Write the `simple/cnab/build/Dockerfile` to build the invocation image
...
**EXERCISE**: Fill the file `simple/cnab/app/run` with an **executable** shell script able to:
...
Let's build :hammer: our invocation image:
```sh
$ cd simple
...
Docker app generated an invocation `FROM` a base docker app invocation image, containing the `run` binary. It copied your application package and all the attachments. The `run` backend nows how to install your package using `docker stack deploy`, and even `render` or `inspect` it. This `bundle` command is executed on-the-fly by each command, without you noticing it.
...
- put our hands on CNAB specifications
- created our own CNAB bundle
- understood what is behind the hood with Docker App CNAB runtime